...
CPICkitFunctions::~CPICkitFunctions() = default;
...
int CPICkitFunctions::ReadPkStatus() {
...
unsigned int CPICkitFunctions::ComputeChecksum() {
    ...
    if (DevFile.Families[ActiveFamily].BlankValue < 0xFFFF) { // 16F and baseline parts are caculated a word at a time.
        ...
        if (((DevFile.PartsList[ActivePart].CPMask &
              DeviceBuffers->ConfigWords[DevFile.PartsList[ActivePart].CPConfig - 1])
        ...
        if (DevFile.PartsList[ActivePart].ConfigWords > 0) {
            ...
            { // if a code protect bit is set, the checksum is computed differently.
                ...
                for (int idx = 0; idx <
                                  DevFile.PartsList[ActivePart].UserIDWords; idx++) { // add last nibble of UserIDs in decreasing nibble positions of checksum
                    int idPosition = 1;
                    for (int factor = 0; factor < idx; factor++) {
                        idPosition <<= 4;
                    }
                    checksum += (unsigned int) (
                            (0xF & DeviceBuffers->UserIDs[DevFile.PartsList[ActivePart].UserIDWords - idx - 1])
                            * idPosition);
                }
            }
        ...
        int progMemEnd =
                (int) DevFile.PartsList[ActivePart].ConfigAddr / DevFile.Families[ActiveFamily].ProgMemHexBytes;
        ...
        if (DevFile.PartsList[ActivePart].ConfigWords > 0) {
            ...
            for (int idx = 0; idx < DevFile.PartsList[ActivePart].ConfigWords; idx++) {
                unsigned int memWord = (DeviceBuffers->ConfigWords[idx] &
                                        DevFile.PartsList[ActivePart].ConfigMasks[idx]);
                checksum += (memWord & 0x000000FF);
                checksum += ((memWord >> 8) & 0x000000FF);
            }
        }
...
unsigned int CPICkitFunctions::ReadDeviceID() {
...
bool CPICkitFunctions::FamilyIsKeeloq() {
...
bool CPICkitFunctions::FamilyIsEEPROM() {
...
bool CPICkitFunctions::FamilyIsMCP() {
...
bool CPICkitFunctions::FamilyIsPIC32() {
...
bool CPICkitFunctions::WriteDevice(bool progmem, bool eemem, bool uidmem, bool cfgmem, bool useLowVoltageRowErase) {
    ...
    if ((configLocation < (int) DevFile.PartsList[ActivePart].ProgramMem) &&
        (configWords > 0)) {// if config in program memory, set them to clear.
    ...
    if (progmem) {
        ...
        if (DevFile.PartsList[ActivePart].ProgMemWrPrepScript !=
            0) { // if prog mem address set script exists for this part
            DownloadAddress3(0);
        ...
        if (useProgExec33()) {
            ...
            do {
                ...
                if (((wordsWritten % 0x8000) == 0) &&
                    (DevFile.PartsList[ActivePart].ProgMemWrPrepScript != 0)) { //PIC24 must update TBLPAG
...
/*    if (configInProgramSpace)
    ...
    if (eemem && (DevFile.PartsList[ActivePart].EEMem > 0)) {
        ...
        // find end of used EE
        if (progmem &&
            !useLowVoltageRowErase) { // we're writing all, so EE is erased first, we can skip blank locations at end
    ...
    // Write UserIDs ---------------------------------------------------------------------------------------------------
    if (uidmem && (DevFile.PartsList[ActivePart].UserIDWords >
                   0)) { // do not write if EE unselected as PIC18F cannot erase/write UserIDs except with ChipErase
        RunScript(SCR_PROG_ENTRY, 1);
    ...
    if (cfgmem) { //
        // Write Configuration
        if ((configWords > 0) &&
            (!configInProgramSpace)) { // Write config words differently for any part where they are stored in program memory.
            ...
            if (_tcsncmp(DevFile.Families[ActiveFamily].FamilyName, "PIC18F", 9) ||
                ...
                if (DevFile.PartsList[ActivePart].ConfigWords >
                    5) { // don't blow up if part doesn't have enough config words
                    if ((DeviceBuffers->ConfigWords[5] & ~0x2000) ==
                        DeviceBuffers->ConfigWords[5]) { // if WRTC is asserted
...
bool CPICkitFunctions::HCS360_361_VppSpecial() {
...
bool CPICkitFunctions::EraseDevice(bool progmem, bool eemem, bool *useLowVoltageRowErase) {
    ...
    *useLowVoltageRowErase = false;
    ...
    if (progmem && (eemem || DevFile.PartsList[ActivePart].EEMem == 0)) { // chip erase when programming all
        if (*useLowVoltageRowErase) { // use row erases
            ...
            RunScript(SCR_PROG_EXIT, 1);
               ...
               (DevFile.PartsList[ActivePart].ProgMemEraseScript != 0)) { // Don't erase EE when not selected.
        if (*useLowVoltageRowErase) { // use row erases
            RowEraseDevice();
        } else { // bulk erases
            RunScript(SCR_PROG_ENTRY, 1);
    ...
    } else if (eemem && (DevFile.PartsList[ActivePart].EEMemEraseScript !=
                         0)) { // Some parts must have EE bulk erased before being re-written.
        RunScript(SCR_PROG_ENTRY, 1);
        RunScript(SCR_ERASE_EE, 1);
        RunScript(SCR_PROG_EXIT, 1);
    } else if ((!eemem && (DevFile.PartsList[ActivePart].EEMem != 0))
               && (DevFile.PartsList[ActivePart].ProgMemEraseScript ==
                   0)) {// Some parts cannot erase ProgMem, UserID, & config without erasing EE
    ...
    if (*useLowVoltageRowErase) {
        ...
        if (DevFile.PartsList[ActivePart].WriteCfgOnErase) {
            ...
            if ((configLocation < DevFile.PartsList[ActivePart].ProgramMem) &&
                (configWords > 0)) {// if config in program memory, set them to clear.
                orMask = 0;
...
void CPICkitFunctions::RowEraseDevice() {
...
bool CPICkitFunctions::SerialEEPROMErase() {
    CDeviceData *saveBuffers = DeviceBuffers;
    DeviceBuffers = BlankBuffers;
...
bool CPICkitFunctions::ReadOSSCAL() {
    if (RunScript(SCR_PROG_ENTRY, 1)) {
...
void CPICkitFunctions::ReadBandGap() {
    RunScript(SCR_PROG_ENTRY, 1);
    ...
    if (progmem) {
        if (useProgExec33()) {
            ...
            if ((DevFile.PartsList[ActivePart].ProgMemAddrSetScript != 0)
                && (DevFile.PartsList[ActivePart].ProgMemAddrBytes !=
                    0)) { // if prog mem address set script exists for this part & # bytes is not zero
            ...
            do {
                if (FamilyIsEEPROM()) {
                    if ((DevFile.PartsList[ActivePart].ConfigMasks[PROTOCOL_CFG] == I2C_BUS)
                        ...
                        && (wordsRead % (DevFile.PartsList[ActivePart].ConfigMasks[ADR_MASK_CFG] + 1) ==
                            0)) { // must resend address to EE every time we cross a bank border.
                ...
                for (int word = 0; word < wordsPerLoop; word++) {
                    ...
                    if (function == READ_MEM) {
                        ...
                        if (DeviceBuffers->ProgramMemory[wordsRead++] != memWord) { // location failed verify
                            if (!DevFile.PartsList[ActivePart].OSSCALSave
                                || (DevFile.PartsList[ActivePart].ProgramMem !=
                                    (unsigned int) wordsRead)) { // skip last byte in memory if it is osccal
                                RunScript(SCR_PROG_EXIT, 1);
                                VddOff();
                                timerStop();
                                _tcsncpy_s(ReadError.memoryType, "Program", 7);
...
bool CPICkitFunctions::BusErrorCheck() {
...
unsigned int CPICkitFunctions::getEEBlank() {
...
void CPICkitFunctions::timerPrint() {
    static char timer = '\\';
    static char count = 0;
...
bool CPICkitFunctions::UploadDataNoLen() {
    unsigned char commandArray[BUF_SIZE];
...
bool CPICkitFunctions::GetUpload() {
    return readUSB();
}

bool CPICkitFunctions::VddOn() {
    unsigned char commandArray[BUF_SIZE];
    commandArray[0] = FWCMD_EXECUTE_SCRIPT;
    commandArray[1] = 2;
...
void CPICkitFunctions::PrepPICkit2() {
    // Set VDD voltage
...
void CPICkitFunctions::ResetBuffers() {
...
float CPICkitFunctions::GetDefaultVdd() { // reads default VDD for active part from device file int vddSetPoint
...
float CPICkitFunctions::GetDefaultVpp() { // reads default VPP for active part from device file into vppSetPoint
...
void CPICkitFunctions::ClosePICkit2Device() {
...
bool CPICkitFunctions::ReadDeviceFile(_TCHAR *path) {
    ...
    if ((err = fopen_s(&datfile, path, "rb")) == 0) {    // Load device file params
        ...
        for (i = 0; i < DevFile.Info.NumberParts; i++) {
            ...
            fread(&DevFile.PartsList[i].Config9Mask, sizeof(DevFile.PartsList[i].Config9Mask), 1,
                  datfile); // Added for compat level 6
            DevFile.PartsList[i].ConfigMasks[8] = DevFile.PartsList[i].Config9Mask;
            fread(&DevFile.PartsList[i].Config9Blank, sizeof(DevFile.PartsList[i].Config9Blank), 1,
                  datfile); // Added for compat level 6
            ...
            fread(&DevFile.PartsList[i].LVPScript, sizeof(DevFile.PartsList[i].LVPScript), 1,
                  datfile); // Added for compat level 6
        }
...
void CPICkitFunctions::downloadPartScripts(int familyIndex) {
    ...
    {
        if (vppFirstEnabled &&
            (DevFile.Families[familyIndex].ProgEntryVPPScript != 0)) { // download VPP first program mode entry
...
unsigned int CPICkitFunctions::getScriptBufferChecksum() {
    unsigned char commandArray[BUF_SIZE];
...
bool CPICkitFunctions::readUSB() {
    return m_Driver.ReadReport((char *) Usb_read_array);
}

void CPICkitFunctions::USBClose() {
    m_Driver.CloseReport();
}
...
bool CPICkitFunctions::EnterUartMode() {
    unsigned char commandArray[BUF_SIZE];
...
bool CPICkitFunctions::BL_EraseFlash() {
    unsigned char commandArray[BUF_SIZE];
...
bool CPICkitFunctions::BL_WriteFWLoadedKey() {
...
bool CPICkitFunctions::BL_Reset() {
    unsigned char commandArray[BUF_SIZE];
...
bool CPICkitFunctions::UnitIDRead(_TCHAR *unitID) { // returns a zero-length string if no ID.
    int i;
...
_TCHAR *CPICkitFunctions::GetUnitID() {
    return m_Driver.GetPK2UnitID();
}

bool CPICkitFunctions::ResetPICkit2() {
    unsigned char commandArray[BUF_SIZE];
...
void CPICkitFunctions::writeConfigInsideProgramMem() {
    int lastBlock;
    int word, bite;
...
bool CPICkitFunctions::FamilyIsPIC24H() {
...
bool CPICkitFunctions::FamilyIsdsPIC33F() {
...
bool CPICkitFunctions::FamilyIsPIC24F() {
...
unsigned int CPICkitFunctions::ReadVector() {
    SetMCLR(true);     // assert /MCLR to prevent code execution before programming mode entered.
    VddOn();
...
void CPICkitFunctions::EnterSerialExecution() { // assumes already in programming mode
    int commOffSet = 0;
...
bool CPICkitFunctions::DownloadPE() {
    CPIC32PE P32PE;
...
int CPICkitFunctions::ReadPEVersion() {
    unsigned char commandArrayp[BUF_SIZE];
    int commOffSet = 0;
...
bool CPICkitFunctions::PE_DownloadAndConnect() {
...
bool CPICkitFunctions::PIC32Read(bool progmem, bool uidmem, bool cfgmem) {
    ...
    if (progmem) {
        ...
        do {
            ...
            for (i = 0; i < runs; i++) {
                commOffSet = addInstruction(commandArrayp, (address + (unsigned int) (i *
                                                                                      DevFile.PartsList[ActivePart].ProgMemRdWords *
                                                                                      bytesPerWord)), commOffSet);
            }
            writeUSB(commandArrayp, commOffSet);
...
bool CPICkitFunctions::PIC32BlankCheck() {
    SetMCLR(true);     // assert /MCLR to prevent code execution before programming mode entered.
    VddOn();
...
bool CPICkitFunctions::P32Write(bool progmem, bool uidmem, bool cfgmem) {
    ...
    if (cfgmem) {
        ...
        cfgBuf[0] |= (~(unsigned int) DevFile.PartsList[ActivePart].ConfigMasks[0] &
                      DevFile.PartsList[ActivePart].ConfigBlank[0])
                     | ((~(unsigned int) DevFile.PartsList[ActivePart].ConfigMasks[1] &
                         DevFile.PartsList[ActivePart].ConfigBlank[1]) << 16);
        cfgBuf[1] = DeviceBuffers->ConfigWords[2] | (DeviceBuffers->ConfigWords[3] << 16);
        cfgBuf[1] |= (~(unsigned int) DevFile.PartsList[ActivePart].ConfigMasks[2] &
                      DevFile.PartsList[ActivePart].ConfigBlank[2])
                     | ((~(unsigned int) DevFile.PartsList[ActivePart].ConfigMasks[3] &
                         DevFile.PartsList[ActivePart].ConfigBlank[3]) << 16);
        cfgBuf[2] = DeviceBuffers->ConfigWords[4] | (DeviceBuffers->ConfigWords[5] << 16);
        cfgBuf[2] |= (~(unsigned int) DevFile.PartsList[ActivePart].ConfigMasks[4] &
                      DevFile.PartsList[ActivePart].ConfigBlank[4])
                     | ((~(unsigned int) DevFile.PartsList[ActivePart].ConfigMasks[5] &
                         DevFile.PartsList[ActivePart].ConfigBlank[5]) << 16);
...
void CPICkitFunctions::PEProgramSendBlock(int index, bool peResp) { // Assumes DL buffer is 256!
...
bool CPICkitFunctions::P32Verify(bool writeVerify, bool progmem, bool uidmem, bool cfgmem) {
    ...
    if (cfgmem) {
        ...
        deviceCRC = PEGetCRC(K_P32_BOOT_FLASH_START_ADDR + (unsigned int) ((bootMemP32 + 1) * bytesPerWord),
 ...
 *********************************************************************************************
 ********************************************************************************************/
void CPICkitFunctions::DisablePE33() {
    usePE33 = false;
}

bool CPICkitFunctions::DownloadPE33() {
    CdsP33_PE P33PE;
...
bool CPICkitFunctions::PE33_Connect() {
    RunScript(SCR_PROG_ENTRY, 1);
...
bool CPICkitFunctions::PE33_DownloadAndConnect() {
    // VDD must already be on!
...
bool CPICkitFunctions::PE33BlankCheck() {
...
bool CPICkitFunctions::PE33Read() {
    if (!PE33_DownloadAndConnect()) {
        return false;
    }
...
bool CPICkitFunctions::PE33VerifyCRC() {
    if (!PE33_DownloadAndConnect()) {
        return false;
    }
...
unsigned short CPICkitFunctions::CalcCRCProgMem() {
...
bool CPICkitFunctions::DownloadPE24F() {
    CP24F_PE P24FPE;
...
bool CPICkitFunctions::PE24F_Connect() {
    RunScript(SCR_PROG_ENTRY, 1);
...
bool CPICkitFunctions::PE24F_DownloadAndConnect() {
    // VDD must already be on!
    // reduce PE comm speed to 500kHz max
    //ICSPSpeedRestore = LastICSPSpeed;
    if (LastICSPSpeed < 2)
...
bool CPICkitFunctions::PE24FBlankCheck() {
    ...
    do {
        ...
        for (word = 0; word < wordsPerLoop; word += 2) {
            ...
            if (wordsRead >= configsStart) {
                blankVal = (0xFF0000 |
                            (unsigned int) DevFile.PartsList[ActivePart].ConfigBlank[wordsRead - configsStart]);
            }
...
bool CPICkitFunctions::PE24FRead() {
    if (!PE24F_DownloadAndConnect()) {
        return false;
    }
...
bool CPICkitFunctions::PE24FWrite(int endOfBuffer) {
    ...
    if (endOfBuffer == (int) DevFile.PartsList[ActivePart].ProgramMem) {// if we'll be writing configs
        ...
        for (cfg = DevFile.PartsList[ActivePart].ConfigWords; cfg > 0; cfg--) {
            DeviceBuffers->ProgramMemory[endOfBuffer - cfg] &=
                    (0xFF0000 | (unsigned int) DevFile.PartsList[ActivePart].ConfigBlank[
                            DevFile.PartsList[ActivePart].ConfigWords - cfg]);
        }
    }
...
bool CPICkitFunctions::useProgExec24F() {
    if (FamilyIsPIC24F()) {
        if (DevFile.PartsList[ActivePart].ProgramMem >= 4096)
            // don't use PE on the smallest parts.
            return usePE33;
