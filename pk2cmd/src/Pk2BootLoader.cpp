        ...
        while (!feof(hexfile)) {
            ...
            if ((fileLine[0] == ':') &&
                ...
                if (recordType == 0) { // Data Record}
                    if ((fileAddress >= 0x2000) &&
                        (fileAddress < 0x7FE0)) { // don't program 5555 key at last address until after verification.
                        if (!second16) {
    ...
    if (_tfopen_s(&hexfile, fileName, "rt") == 0) {
        ...
        while (!feof(hexfile)) {
            ...
            if ((fileLine[0] == ':') &&
                ...
                if (recordType == 0) { // Data Record}
                    if ((fileAddress >= 0x2000) && (fileAddress < 0x7FE0)) { // don't check bootloader stuff.
                        int startByte = fileAddress & 0x000F; // read 16 bytes at a time.
                        int firstAddress = fileAddress & 0xFFF0;
                        if (lastAddress != firstAddress) { // only read if next line in different 16-byte block
                        ...
                        if ((int) _tcslen(fileLine) >=
                            ...
                            for (int lineByte = 0; lineByte < byteCount; lineByte++) {
                                ...
                                if (picFuncs->Usb_read_array[5 + startByte + lineByte] !=
                                    (unsigned char) (wordByte & 0xFF)) {
                                    verified = false;
                                    recordType = 1;
                                    break;
                                }
