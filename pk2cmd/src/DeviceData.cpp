...
CDeviceData::CDeviceData() = default;
...
void CDeviceData::ClearConfigWords(unsigned char numConfigs, const unsigned short int configBlank[]) {
    if (numConfigs > 0) {
...
void CDeviceData::ClearEEPromMemory(unsigned short int eeMemSize, int eeBytes, unsigned int memBlankVal) {
    if (eeMemSize > 0) {
        ...
        if (memBlankVal == 0xFFF) { // baseline dataflash
            eeBlankVal = 0xFFF;
        }
