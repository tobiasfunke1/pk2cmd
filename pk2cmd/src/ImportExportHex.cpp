...
bool CImportExportHex::ImportHexFile(_TCHAR *filePath, CPICkitFunctions *picFuncs) {
    ...
    if ((err = _tfopen_s(&hexfile, filePath, "rt")) == 0) {
        ...
        while (!feof(hexfile)) {
            ...
            if ((fileLine[0] == ':') &&
                (_tcslen(fileLine) >= 11)) { // skip line if not hex line entry,or not minimum length ":BBAAAATTCC"
                ...
                if (recordType == 0) { // Data Record}
                    if ((int) _tcslen(fileLine) >=
                        (11 + (2 * byteCount))) { // skip if line isn't long enough for bytecount.
                        ...
                        for (int lineByte = 0; lineByte < byteCount; lineByte++) {
                            ...
                            for (int shift = 0; shift < bytePosition; shift++) { // shift byte into proper position
                                wordByte <<= 8;
                            ...
                            // boot memory section --------------------------------------------------
                            if ((bootMemSize > 0) && (byteAddress >= (int) bootMemStart) &&
                                (byteAddress < (int) bootMemEnd)) {
                            ...
                            else if ((byteAddress >= (int) eeAddr) && (eeAddr > 0) &&
                                     ((int) picFuncs->DevFile.PartsList[picFuncs->ActivePart].EEMem == 0)) {
                            ...
                            if ((byteAddress >= (int) picFuncs->DevFile.PartsList[picFuncs->ActivePart].ConfigAddr)
                                && (configWords > 0)) {
                                int configNum = (byteAddress -
                                                 ((int) picFuncs->DevFile.PartsList[picFuncs->ActivePart].ConfigAddr)) /
                                                cfgBytesPerWord;
                                if ((cfgBytesPerWord != bytesPerWord) && (bytePosition > 1)) { // PIC32
                                    wordByte = (wordByte >> 16) &
                                               picFuncs->DevFile.Families[picFuncs->ActiveFamily].BlankValue;
                                }
                                if (configNum < picFuncs->DevFile.PartsList[picFuncs->ActivePart].ConfigWords) {
                                    ...
                                    if (byteAddress <
                                        progMemSizeBytes) { // also mask off the word if in program memory.
                                        picFuncs->DeviceBuffers->ProgramMemory[arrayAddress] &=
                                                (wordByte &
                                                 picFuncs->DevFile.PartsList[picFuncs->ActivePart].ConfigMasks[configNum]); // add byte.
                                        if (picFuncs->DevFile.Families[picFuncs->ActiveFamily].BlankValue ==
                                            0xFFFF) {// PIC18J
                                            picFuncs->DeviceBuffers->ProgramMemory[arrayAddress] |= 0xF000; // set upper nibble
                                        } else {// PIC24FJ is currently only other case of config in program mem
                            ...
                            if (userIDs > 0) {
                                if (byteAddress >= (int) userIDAddr) {
                                    ...
                                    if (uIDAddress < userIDs) {
                                        ...
                                        } else {  // PIC18F/J, PIC24H/dsPIC33
                                            int uIDshift = (bytePosition / userIDMemBytes) * userIDMemBytes;
                                            for (int reshift = 0;
                                                 reshift < uIDshift; reshift++) { // shift byte into proper position
                                                wordByte >>= 8;
                                            }
                                            picFuncs->DeviceBuffers->UserIDs[uIDAddress] &= wordByte; // add byte.
                                        }
                            ...
                            if (picFuncs->DevFile.PartsList[picFuncs->ActivePart].IgnoreBytes > 0) {
                                if (byteAddress >=
                                    (int) picFuncs->DevFile.PartsList[picFuncs->ActivePart].IgnoreAddress) {
                                    if (byteAddress <
                                        (int) (picFuncs->DevFile.PartsList[picFuncs->ActivePart].IgnoreAddress
                                               +
                                               picFuncs->DevFile.PartsList[picFuncs->ActivePart].IgnoreBytes)) { // if data is in the ignore region, don't do anything with it
                ...
                if ((recordType == 2) || (recordType == 4)) { // Segment address
                    if ((int) _tcslen(fileLine) >=
                        (11 + (2 * byteCount))) { // skip if line isn't long enough for bytecount.
...
bool CImportExportHex::ImportBINFile(_TCHAR *filePath, CPICkitFunctions *picFuncs) { // for serial EEPROMS only
    FILE *binfile;
    errno_t err;
...
bool CImportExportHex::ExportBINFile(_TCHAR *filePath, CPICkitFunctions *picFuncs) { // for serial EEPROMS only
    unsigned char fileByte = 0;
    FILE *binfile;
    errno_t err;
...
unsigned char CImportExportHex::computeChecksum(_TCHAR *fileLine) {
    int byteCount = ParseHex(&fileLine[1], 2);
    if ((int) _tcslen(fileLine) >= (9 + (2 * byteCount))) { // skip if line isn't long enough for bytecount.
