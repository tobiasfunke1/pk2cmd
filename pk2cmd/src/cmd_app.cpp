...
void Ccmd_app::PK2_CMD_Entry(int argc, _TCHAR *argv[]) {
    ...
    if (checkDevFilePathOptionB(argc, argv, tempString)) { // check for explicit path with -B
        if (tempString[_tcslen(tempString) - 1] != '/')
            _tcscat_s(tempString, "/PK2DeviceFile.dat");
        else
            _tcscat_s(tempString, "PK2DeviceFile.dat");
    } else if (ReturnCode == INVALID_CMDLINE_ARG) { // -B, but bad path
        return;
    } else { // no -B, check PATH
...
bool Ccmd_app::Pk2OperationCheck(int argc, _TCHAR *argv[]) {
    ...
    for (i = 1; i < argc; i++) {
        if (checkSwitch(argv[i])) {
            switch (argv[i][1]) { // these options require PICkit 2 communications.
                case 'A':
                case 'a':
                case 'C':
                case 'c':
...
void Ccmd_app::processArgs(int argc, _TCHAR *argv[]) {
    ...
    // auto detect?
    if (tempString[0] == 0) { // no argument, full autodetect
        if (detectAllFamilies(argc, argv)) { // found a device
            _tcsncpy_s(tempString, PicFuncs.DevFile.PartsList[PicFuncs.ActivePart].PartName, 28);
            printf("Auto-Detect: Found part %s.\n\n", tempString);
        } else if (ReturnCode == INVALID_CMDLINE_ARG) { // arg error
            return;
        } else {
    ...
    } else if (((tempString[0] == 'f') || (tempString[0] == 'F')) && (tempString[1] == 0)) { // print family ID numbers
        printFamilies();
        return;
    } else if (((tempString[0] == 'f') || (tempString[0] == 'F')) && (tempString[1] != 0)) { // auto detect family
        if (detectSpecificFamily(&tempString[1], argc, argv)) { // found a device
            _tcsncpy_s(tempString, PicFuncs.DevFile.PartsList[PicFuncs.ActivePart].PartName, 28);
            printf("Auto-Detect found part %s.\n\n", tempString);
        } else if (ReturnCode == INVALID_CMDLINE_ARG) { // arg error
            return;
...
bool Ccmd_app::detectAllFamilies(int argc, _TCHAR *argv[]) {
    ...
    for (int searchIndex = 0; searchIndex < PicFuncs.DevFile.Info.NumberFamilies; searchIndex++) {
        for (int searchPriority = 0; searchPriority < PicFuncs.DevFile.Info.NumberFamilies; searchPriority++) {
            if ((PicFuncs.DevFile.Families[searchPriority].PartDetect) &&
                (PicFuncs.DevFile.Families[searchPriority].SearchPriority == searchIndex)) {
...
bool Ccmd_app::bootloadArg(int argc, _TCHAR *argv[]) {
    ...
    for (i = 1; i < argc; i++) {
        ...
        if ((checkSwitch(argv[i])) && ((argv[i][1] == 'D') || (argv[i][1] == 'd'))) {
            ...
            argv[i] = (char *) "";
            j = 1;
            while (((i + j) < argc) && (!checkSwitch(argv[i + j]))) { // check for path with space(s) in it
...
bool Ccmd_app::unitIDArg(int argc, _TCHAR *argv[]) {
    ...
    for (i = 1; i < argc; i++) {
        ...
        if ((checkSwitch(argv[i])) && ((argv[i][1] == 'N') || (argv[i][1] == 'n'))) {
            ...
            argv[i] = (char *) "";
            j = 1;
            while (((i + j) < argc) && (!checkSwitch(argv[i + j]))) { // check for name with space(s) in it
...
bool Ccmd_app::selectUnitArg(int argc, _TCHAR *argv[]) {
    ...
    for (i = 1; i < argc; i++) {
        ...
        if ((checkSwitch(argv[i])) && ((argv[i][1] == 'S') || (argv[i][1] == 's'))) {
            ...
            if ((_tcslen(argv[i]) > 2) && !listFWVer) { // find specific unit
...
bool
Ccmd_app::priority1Args(int argc, _TCHAR *argv[], bool preserveArgs) {    // returns false if any command has an error.
            ...
            switch (argv[i][1]) {
                ...
                case 'f':
                    if (!preserveArgs) // skip if still looking for a part
                    {
                        ...
                        if (j >
                            3) { // this is kind of brute force, but avoids a lot of string library calls and another tempstring
                ...
                case 'l':
                    // Set ICSP speed
                    if (argv[i][2] == 0) { // no specified value - illegal
                    ...
                    } else { // no specified value - illegal
                        printf("-L Invalid value.\n");
                        fflush(stdout);
                        ret = false;
                        ReturnCode = INVALID_CMDLINE_ARG;
                ...
                case 'v':
                    // VPP override
                    if (preserveArgs) { // cannot be used with part detect
...
bool Ccmd_app::checkArgsForBlankCheck(int argc, _TCHAR *argv[]) {    // returns false if there is an error.
...
bool Ccmd_app::priority2Args(int argc, _TCHAR *argv[]) {    // returns false if any command has an error.
        ...
        if ((checkSwitch(argv[i])) && ((argv[i][1] == 'E') || (argv[i][1] == 'e')) && ret) {
            if (PicFuncs.FamilyIsKeeloq()) {
                ...
                ret = false;
                       ...
                       && (PicFuncs.DevFile.PartsList[PicFuncs.ActivePart].ConfigMasks[PROTOCOL_CFG] !=
                           UNIO_BUS)) { // Microwire / UNIO have a true "chip erase".  Other devices must write every byte blank.
    ...
    for (i = 1; i < argc; i++) {
        ...
        if ((checkSwitch(argv[i])) && ((argv[i][1] == 'M') || (argv[i][1] == 'm')) && ret) {
            if (hexLoaded) {
                ...
                if (argv[i][2] == 0) { // no specified region - erase then program all
                    if (PicFuncs.FamilyIsEEPROM()) {
                        ...
                        if (PicFuncs.FamilyIsPIC32()) {
                            // Program everything.
                            noProgEntryForVerify = PicFuncs.WriteDevice(program, eedata, userid, config,
                                                                        usingLowVoltageErase);
                        } else {
                            // program all but configs and verify, as configs may contain code protect
                            noProgEntryForVerify = PicFuncs.WriteDevice(program, eedata, userid, false,
                                                                        usingLowVoltageErase);
                        }
                        if (!noProgEntryForVerify) { // if it is true, then configs are in program memory
                            ret = PicFuncs.ReadDevice(VERIFY_MEM_SHORT, program, eedata, userid, false);
                            verify = ret;
                            argError = ret;
                            if (ret) { // now program configs
                                program = false;
                                eedata = false;
                                userid = false;
                                noProgEntryForVerify = PicFuncs.WriteDevice(program, eedata, userid, config,
                        ...
                        switch (argv[i][j]) {
                            ...
                            case 'C':
                                ...
                                if (PicFuncs.DevFile.PartsList[PicFuncs.ActivePart].ConfigWords > 0) {
                                    int configLocation =
                                            (int) PicFuncs.DevFile.PartsList[PicFuncs.ActivePart].ConfigAddr /
                                    ...
                                    if ((configLocation <
                                         (int) PicFuncs.DevFile.PartsList[PicFuncs.ActivePart].ProgramMem) &&
                                        (configWords > 0)) {
                            ...
                            case 'V': {
                                ...
                                argv[i] = (char *) "";
                                int k = 1;
                                if (((i + k) < argc) && (!checkSwitch(argv[i + k]))) { // check for space after v
        ...
        if ((checkSwitch(argv[i])) && ((argv[i][1] == 'Y') || (argv[i][1] == 'y')) && ret) {
            if (hexLoaded) {
                ...
                if (PicFuncs.FamilyIsKeeloq()) {
                    ...
                    fflush(stdout);
                    ret = false;
                } else if (argv[i][2] == 0) { // no specified region - verify all
                    program = true;
                    eedata = true;
                    userid = true;
                    config = true;
    ...
    for (i = 1; i < argc; i++) {
        ...
        if ((checkSwitch(argv[i])) && ((argv[i][1] == 'G') || (argv[i][1] == 'g')) && ret) {
            ...
            if (argv[i][2] == 0) { // no specified type - illegal
                ret = false;
                ...
                switch (argv[i][2]) {
                    ...
                    case 'F':
                        if (PicFuncs.ReadDevice(READ_MEM, true, true, true, true)) {
                            ...
                            j = 1;
                            while (((i + j) < argc) &&
                                   (!checkSwitch(argv[i + j]))) { // check for path with space(s) in it
                                if (_tcslen(tempString) !=
                                    0) // don't add space if it's between "F" and start of filename
                                    _tcscat_s(tempString, " ");
                                _tcscat_s(tempString, argv[i + j]);
                                argv[i + j++] = (char *) "";
                            }
                    ...
                    case 'P':
                        ...
                        argv[i] = (char *) "";
                        j = 1;
                        if (((i + j) < argc) && (!checkSwitch(argv[i + j]))) { // check for space after p
                            _tcscat_s(tempString, argv[i + j]);
                            argv[i + j++] = (char *) "";
                        }
                    ...
                    case 'C':
                        if (PicFuncs.DevFile.PartsList[PicFuncs.ActivePart].ConfigWords > 0) {
                            ...
                            int configLocation = (int) PicFuncs.DevFile.PartsList[PicFuncs.ActivePart].ConfigAddr /
                                                 PicFuncs.DevFile.Families[PicFuncs.ActiveFamily].ProgMemHexBytes;
                            int configWords = PicFuncs.DevFile.PartsList[PicFuncs.ActivePart].ConfigWords;
                            if ((configLocation < (int) PicFuncs.DevFile.PartsList[PicFuncs.ActivePart].ProgramMem) &&
                                (configWords > 0)) {
                                printf("This device has configuration words in Program Memory.\n");
                                fflush(stdout);
                                ReturnCode = INVALID_CMDLINE_ARG;
...
bool Ccmd_app::priority3Args(int argc, _TCHAR *argv[]) {    // returns false if any command has an error.
                    ...
                    if (PicFuncs.DevFile.Families[PicFuncs.ActiveFamily].PartDetect) {
                        ...
                        if (deviceID == 0) {
                            ...
                            for (j = 1; j < PicFuncs.DevFile.Info.NumberParts; j++) {
                                if ((PicFuncs.DevFile.PartsList[j].Family == PicFuncs.ActiveFamily) &&
                                    (deviceID == (int) PicFuncs.DevFile.PartsList[j].DeviceID) &&
                                    PicFuncs.DevFile.Families[PicFuncs.ActiveFamily].PartDetect) {
                            ...
                            if (j == PicFuncs.DevFile.Info.NumberParts) { // no matching device found.
...
bool Ccmd_app::priority4Args(int argc, _TCHAR *argv[]) {    // returns false if any command has an error.
...
bool Ccmd_app::getRange(int *start, int *stop, const _TCHAR *str_range) {
    ...
    if (*str_range == 0) { // no range, return (use defaults)
        return true;
    }
...
bool Ccmd_app::getValue(unsigned int *value, const _TCHAR *str_value) {
    ...
    if (*str_value == 0) { // no value, return error
        return false;
    }
...
bool Ccmd_app::checkHelp1(int argc, _TCHAR *argv[]) { // Helps that don't need the device file.
    int i;
...
bool
Ccmd_app::checkHelp2(int argc, _TCHAR *argv[], bool loadDeviceFileFailed) {  // helps that need the device file loaded
    int i;
                ...
                if (_tcslen(argv[i]) > 2) {
                    if ((argv[i][2] == 'v') || (argv[i][2] == 'V')) {
                        ...
                        if (loadDeviceFileFailed) {
                            ...
                            int j = 1;
                            while (((i + j) < argc) &&
                                   (!checkSwitch(argv[i + j]))) { // check for term with space(s) in it
    ...
    for (int index = 0; index < PicFuncs.DevFile.Info.NumberFamilies; index++) {
        for (int order = 0; order < PicFuncs.DevFile.Info.NumberFamilies; order++) {
            if (PicFuncs.DevFile.Families[order].FamilyType == index) {
                ...
                // list them
                if (argSearch[0] == 0) { // list all parts
