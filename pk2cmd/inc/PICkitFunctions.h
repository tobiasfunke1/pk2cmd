...
public:
    ...
    unsigned int GetDeviceRevision() const;
    ...
    bool HCS360_361_VppSpecial();

    void WriteConfigOutsideProgMem();

    int DataClrAndDownload(const unsigned char dataArray[], int arrayLength, int startIndex);

    int DataDownload(const unsigned char dataArray[], int arrayLength, int startIndex);

    int FindLastUsedInBuffer(const unsigned int bufferToSearch[], unsigned int blankValue, int startIndex);
    ...
    bool SendScript(const unsigned char script[], int length);
    ...
    static void ArrayCopy(const unsigned char *source, int sourceStart, unsigned char *dest, int destStart, int length);
    ...
    void ClosePICkit2Device();

    static void KillSpace(_TCHAR *name, int maxlength);
    ...
    bool GetVppFirstEnable() const;

    void SetSelfPowered(bool set);

    bool GetSelfPowered() const;
    ...
    bool VerifyBootloaderMode();

    bool BL_EraseFlash();

    bool BL_WriteFlash(const unsigned char payload[]);

    bool BL_WriteFWLoadedKey();
    ...
    bool UnitIDWrite(const _TCHAR *unitID);
    ...
    unsigned int ReadVector();

    void WriteVector(int vtop, int vbot);

    void DisablePE33(); // dsP33PE
    bool useProgExec33(); // dsP33PE
protected:
    ...
    unsigned int getScriptBufferChecksum();

    bool downloadScript(unsigned char scriptBufferLocation, int scriptArrayIndex);

    bool writeUSB(const unsigned char commandList[], int commandLength);
    ...
    void EnterSerialExecution();                    //PIC32
    bool DownloadPE();                            //PIC32
    ...
    int p32CRC_buf(const unsigned int *buffer, unsigned int startIdx, unsigned int len); //PIC32
    bool DownloadPE33(); //dsP33PE
    bool PE33_Connect(); // dsP33PE
    bool PE33_DownloadAndConnect(); // dsP33PE
    bool PE33DoBlankCheck(unsigned int lengthWords); // dsP33PE
    bool PE33BlankCheck(); // dsP33PE
    bool PE33Read(); // dsP33PE
...
protected:
    ...
    struct scriptRedirect {
        ...
        int deviceFileScriptNumber;
    } scriptRedirectTable[SCRIPT_REDIR_TABLE_LEN]{};
};
